...
import type { Command, DocumentModel, ObjectId } from '@vectoria/core';
...
export const RightDock: React.FC<RightDockProps> = ({ document: doc, selectedObjectId, historyEntries, onSelectObject, onUpdatePosition, onUpdateDimensions, onUpdateFill, open }) => {
  ...
  return (
    <aside className={`right-dock ${open ? '' : 'is-closed'}`} data-testid="right-dock">
      ...
      <div id={`panel-${activePanel}`} role="tabpanel" aria-labelledby={`tab-${activePanel}`} className="dock-panel">
        {activePanel === 'properties' && <PropertiesPanel document={doc} selectedObjectId={selectedObjectId} onUpdatePosition={onUpdatePosition} onUpdateDimensions={onUpdateDimensions} onUpdateFill={onUpdateFill} />}
        {activePanel === 'layers' && <LayersPanel document={doc} selectedObjectId={selectedObjectId} onSelectObject={onSelectObject} />}
